...
@section('title_share')
    {{ $mobil_baru->merek->merek }} {{ $mobil_baru->design->design }} {{ $mobil_baru->tipe->tipe }} -
    {{ $spec->city->city }}
@endsection
...
@endsection
@section('description')
    Pilih mobil idaman, apply sekarang juga. Hub Marketing {{ $mobil_baru->merek->merek }} {{ $spec->city->city }} :
    {{ $spec->user->name }} HP: {{ $spec->user->phone }}
@endsection
@section('author')
    {{ $spec->user->name }}
@endsection
...
@section('meta_facebook')
    ...
    <meta property="og:title"
        content="{{ $mobil_baru->merek->merek }} {{ $mobil_baru->design->design }} {{ $mobil_baru->tipe->tipe }} - {{ $spec->city->city }}" />
    <meta property="og:description"
        content="Pilih mobil idaman, apply sekarang juga. Hub Marketing {{ $mobil_baru->merek->merek }} {{ $spec->city->city }} : {{ $spec->user->name }} HP: {{ $spec->user->phone }}" />
            ...
            <div class="container">
                ...
                <article class="single-vehicle-details">
                    <div class="single-vehicle-title">
                        <span class="badge-premium-listing">{{ $mobil_baru->merek->merek }} |
                            {{ $spec->city->city }}</span>
                        <h2 class="post-title">{{ $mobil_baru->merek->merek }} {{ $mobil_baru->design->design }}
                            {{ $mobil_baru->tipe->tipe }}</h2>
                    </div>
                    <div class="single-listing-actions">
                        <div class="btn-group pull-right" role="group">
                            <a href="#" class="btn btn-default" title="Download Manual"><i class="fa fa-book"></i>
                                <span>Download Brosur</span></a>
                        </div>
                        <div class="btn btn-info price">Rp.
                            {{ number_format(isset($price->harga) ? $price->harga : 0, 0, ',', '.') }} /
                            {{ $spec->city->city }}</div>
                    </div>
                    <div class="row">
                        <div class="col-md-8">
                            <div class="single-listing-images">
                                <div class="featured-image format-image">
                                    <a href="/img/mobil-baru/{{ $mobil_baru->foto }}" data-rel="prettyPhoto[gallery]"
                                        class="media-box"><img src="/img/mobil-baru/{{ $mobil_baru->foto }}"
                                            alt=""></a>
                                </div>
                                <div class="additional-images">
                                    <ul class="owl-carousel" data-columns="4" data-pagination="no" data-arrows="yes"
                                        ...
                                        @foreach ($gallery as $value)
                                            <li class="item format-image"> <a href="/img/gallery/{{ $value->gallery }}"
                                                    data-rel="prettyPhoto[gallery]" class="media-box"><img
                                                        src="/img/gallery/{{ $value->gallery }}" alt=""></a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="sidebar-widget widget">
                                <ul class="list-group">
                                    <li class="list-group-item"> <span class="badge">Merek</span>
                                        {{ $mobil_baru->merek->merek }}</li>
                                    <li class="list-group-item"> <span class="badge">Model</span>
                                        {{ $mobil_baru->design->design }}</li>
                                    <li class="list-group-item"> <span class="badge">Tipe</span>
                                        {{ $mobil_baru->tipe->tipe }}</li>
                                    <li class="list-group-item"> <span class="badge">Negara Pembuat</span>
                                        {{ $mobil_baru->negara_pembuat }}</li>
                            ...
                            <div class="sidebar-widget widget seller-contact-widget">
                                <div class="vehicle-enquiry-foot">
                                    ...
                                    <center>
                                        <img src="{{ TutaComp::getImage($spec->user->foto) }}" class="img-thumbnail"
                                            alt="{{ $spec->user->name }}" width="250px">
                                        <br />
